...
/*   Updated: 2021/12/01 17:05:43 by jmurovec      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
//		   Xr  Xi  *  Xr  Xi  +	 Cr  Ci
// f(x) = (1 + 1i) * (1 + 1i) + (1 + 1i) 
int	find_mandelbrot(t_window *w)
{
	int		i;
	double	real;
	double	imgn;
	...
	{
		temp = w->range.c_real + real * real - imgn * imgn;
		imgn = w->range.c_imgn + 2 * real * imgn;
		real = temp;
		i++;
	}
	return (i);
...
void	draw_image_mandelbrot(t_window *w)
{
	...
	while (y < w->image.height)
	{
		...
		while (x < w->image.width)
		{
			...
			w->range.nr_iterations = find_mandelbrot(w);
			w->color_scheme(w);
