...
{
	printf("Key in Win1: ESC (%d)\n", key);
	if (key == 0xFF1B)					// 65307 == ESC
		exit(0);
}

int	key_win2(int key, void *p)
{
	printf("Key in Win2: ESC (%d)\n", key);
	if (key == 0xFF1B)
		exit(0);
}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	printf(" => MAKING FIRST WINDOW (%dx%d) ...",WIN1_SX, WIN1_SY);				// FIRST WINDOW
	if (!(win1 = mlx_new_window(mlx, WIN1_SX, WIN1_SY, "WINDOW 1")))
		{
			printf(" !! KO !!\n");
			exit(1);
		}
	...
	printf("     => Colormap without events ... 10s, ");										// FIRST EVENT
	color_map_1(win1, WIN1_SX, WIN1_SY);
	printf("OK\n");
	sleep(0);

	printf("     => Clear Window ... 3s, ");
	mlx_clear_window(mlx,win1);
	printf("OK\n");
	sleep(0);
																				//////////////////////////////////////////////
	printf(" => Making Image 1: ZPixmap (%dx%d) ... 10s\n", IM1_SX, IM1_SY);						// IMAGE 01
	...
	printf("       OK (pixmap : %d)\n",(int)((t_img *)im1)->pix);						// WHERE THIS COMES FROM ??
	...
	sleep(0);
																				///////////////////////////////////////////////
	printf(" => Image3 ZPixmap %dx%d ...",IM3_SX, IM3_SY);						// IMAGE 02
...
//	 xpm1_x, xpm1_y, bpp2, sl2, endian2, ((t_img *)im2)->type);
	...
	if (!(im4 = mlx_new_image(mlx, IM3_SX, IM3_SY)))								// FIRST IMAGE
		{
			printf(" !! KO !!\n");
			exit(1);
		}
